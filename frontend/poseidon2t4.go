...
// =============================================================================
//
// This file implements the Noir/Barretenberg-compatible Poseidon2T4 hash function used across
// circuits. It is implemented directly in terms of gnark's `frontend.API` operations so that it
// can be used inside circuit `Define` methods.
//
// High level structure:
// - Constants: round constants and internal matrix diagonal (encoded as hex strings)
// - init(): parses constants into `*big.Int` for use as field constants
// - Permutation: external/internal linear layers and the full/partial round schedule
// - Sponge: absorb input rate=3, capacity=1, return s0 as output
//
// Notes:
// - This file intentionally contains a lot of constant data. Organization is primarily for
//   readability and reviewability.
// - `Poseidon2T4(api)` takes a variadic list of inputs; higher-level wrappers should be added
//   at call sites if you want fixed arity for readability.

const poseidon2T4TwoPow64Dec = "18446744073709551616"

// =============================================================================
// Round constants (encoded)
// =============================================================================

var poseidon2T4InternalMatrixDiagonalHex = [4]string{
...
var poseidon2T4FullRounds0To3 = [4][4]string{
	{
...
var poseidon2T4PartialRounds4To59 = [56]string{
...
var poseidon2T4FullRounds60To63 = [4][4]string{
	{
...
var (
	poseidon2T4TwoPow64       *big.Int
	poseidon2T4InternalDiag   [4]*big.Int
	poseidon2T4RoundConstants [64][4]*big.Int
)
...
func init() {
	// Parse constants once at init-time so circuit code can treat them as field constants.
	poseidon2T4TwoPow64 = mustBigHex(poseidon2T4TwoPow64Dec)

	for i := 0; i < 4; i++ {
		poseidon2T4InternalDiag[i] = mustBigHex(poseidon2T4InternalMatrixDiagonalHex[i])
	}

	zero := big.NewInt(0)
	for r := 0; r < 64; r++ {
		for i := 0; i < 4; i++ {
			poseidon2T4RoundConstants[r][i] = zero
		}
	}

	for r := 0; r < 4; r++ {
		for i := 0; i < 4; i++ {
			poseidon2T4RoundConstants[r][i] = mustBigHex(poseidon2T4FullRounds0To3[r][i])
		}
	}

	for i, rc := range poseidon2T4PartialRounds4To59 {
		poseidon2T4RoundConstants[4+i][0] = mustBigHex(rc)
	}

	for r := 0; r < 4; r++ {
		for i := 0; i < 4; i++ {
			poseidon2T4RoundConstants[60+r][i] = mustBigHex(poseidon2T4FullRounds60To63[r][i])
		}
	}
}
...
func poseidon2T4Pow5(api frontend.API, x frontend.Variable) frontend.Variable {
	// x^5 via square-and-multiply: x^2, x^4, x^5.
	x2 := api.Mul(x, x)
	x4 := api.Mul(x2, x2)
	return api.Mul(x4, x)
}

func poseidon2T4Double(api frontend.API, x frontend.Variable) frontend.Variable {
	return api.Add(x, x)
}

func poseidon2T4Quad(api frontend.API, x frontend.Variable) frontend.Variable {
	d := poseidon2T4Double(api, x)
	return poseidon2T4Double(api, d)
}
...
func poseidon2T4ExternalLinearLayer(api frontend.API, s0, s1, s2, s3 frontend.Variable) (frontend.Variable, frontend.Variable, frontend.Variable, frontend.Variable) {
	// External linear layer for the 4-element state.
	t0 := api.Add(s0, s1)
	t1 := api.Add(s2, s3)

	t2 := api.Add(poseidon2T4Double(api, s1), t1)
	t3 := api.Add(poseidon2T4Double(api, s3), t0)

	t4 := api.Add(poseidon2T4Quad(api, t1), t3)
	t5 := api.Add(poseidon2T4Quad(api, t0), t2)
...
func poseidon2T4InternalLinearLayer(api frontend.API, s0, s1, s2, s3 frontend.Variable) (frontend.Variable, frontend.Variable, frontend.Variable, frontend.Variable) {
	// Internal linear layer uses a diagonal matrix plus the sum of all state elements.
	sum := api.Add(api.Add(s0, s1), api.Add(s2, s3))
	ns0 := api.Add(api.Mul(s0, poseidon2T4InternalDiag[0]), sum)
	ns1 := api.Add(api.Mul(s1, poseidon2T4InternalDiag[1]), sum)
	ns2 := api.Add(api.Mul(s2, poseidon2T4InternalDiag[2]), sum)
	ns3 := api.Add(api.Mul(s3, poseidon2T4InternalDiag[3]), sum)
	return ns0, ns1, ns2, ns3
}
...
func poseidon2T4Permute(api frontend.API, s0, s1, s2, s3 frontend.Variable) (frontend.Variable, frontend.Variable, frontend.Variable, frontend.Variable) {
	// Initial external linear layer
	s0, s1, s2, s3 = poseidon2T4ExternalLinearLayer(api, s0, s1, s2, s3)

	// Full rounds 0..3
	for r := 0; r < 4; r++ {
		// Add round constants to all lanes.
		s0 = api.Add(s0, poseidon2T4RoundConstants[r][0])
		s1 = api.Add(s1, poseidon2T4RoundConstants[r][1])
		s2 = api.Add(s2, poseidon2T4RoundConstants[r][2])
		s3 = api.Add(s3, poseidon2T4RoundConstants[r][3])

		// Apply S-box (x^5) to all lanes.
		s0 = poseidon2T4Pow5(api, s0)
		s1 = poseidon2T4Pow5(api, s1)
		s2 = poseidon2T4Pow5(api, s2)
		s3 = poseidon2T4Pow5(api, s3)
	...
	for r := 4; r < 60; r++ {
		...
		// Apply S-box to s0 only (partial rounds).
		s0 = poseidon2T4Pow5(api, s0)

		// Mix lanes with the internal linear layer.
		s0, s1, s2, s3 = poseidon2T4InternalLinearLayer(api, s0, s1, s2, s3)
	}
...
// Poseidon2T4 hashes inputs using the Noir/Barretenberg-compatible Poseidon2T4 sponge.
//
// Behavior:
//   - If called with no inputs (n=0), it still permutes once with IV=0 and returns a fixed non-zero value
//     (in this protocol/field: 11250791130336988991462250958918728798886439319225016858543557054782819955502).
//   - The IV is encoded as n * 2^64 in the capacity element (s3).
//   - The sponge absorbs 3 field elements per permutation and returns s0 as the output.
func Poseidon2T4(api frontend.API, inputs ...frontend.Variable) frontend.Variable {
	n := len(inputs)
	if n == 0 {
		// Keep behavior aligned with the native sponge: squeezing with zero absorbed elements still
		// permutes once (IV=0) and returns a fixed non-zero value.
		s0, s1, s2, s3 := frontend.Variable(0), frontend.Variable(0), frontend.Variable(0), frontend.Variable(0)
		s0, _, _, _ = poseidon2T4Permute(api, s0, s1, s2, s3)
		return s0
	}

	// Initialize the state with an IV in the capacity lane.
	iv := new(big.Int).Mul(big.NewInt(int64(n)), poseidon2T4TwoPow64)
	var s0, s1, s2, s3 frontend.Variable = 0, 0, 0, iv

	for i := 0; i < n; i++ {
		// Absorb 3 inputs per permutation (rate=3).
		switch i % 3 {
		case 0:
			s0 = api.Add(s0, inputs[i])
		case 1:
			s1 = api.Add(s1, inputs[i])
		case 2:
			s2 = api.Add(s2, inputs[i])
		}

		// Permute after absorbing a full rate block, or after the final element.
		if (i%3) == 2 || i == n-1 {
			s0, s1, s2, s3 = poseidon2T4Permute(api, s0, s1, s2, s3)
		}
	}

	// Return the first lane as the sponge output.
	return s0
}
