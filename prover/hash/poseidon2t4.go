...
// Poseidon2T4 implementation compatible with Noir/Barretenberg.
// Parameters: t=4, RF=8, RP=56, d=5
// Sponge construction with IV = input_length * 2^64 (stored in state[3]).
...
const (
	t4Width         = 4  // state size
	t4FullRounds    = 8  // full rounds
	t4PartialRounds = 56 // partial rounds
	t4Rate          = 3  // rate (t - capacity)
)

const twoPow64Dec = "18446744073709551616"

var internalMatrixDiagonalHex = [4]string{
...
var partialRounds4To59 = [56]string{
...
var fullRounds60To63 = [4][4]string{
	{
...
var (
	// 2^64 for IV calculation
	twoPow64 fr.Element

	// Internal matrix diagonal for t=4.
	// From Barretenberg: internal_matrix_diagonal
	internalMatrixDiagonalT4 [t4Width]fr.Element

	// Round constants (64 rounds × 4 elements).
	roundConstantsT4 [t4FullRounds + t4PartialRounds][t4Width]fr.Element
)

func init() {
	// Initialize 2^64
	if _, err := twoPow64.SetString(twoPow64Dec); err != nil {
		panic(err)
	}

	// Internal matrix diagonal
	for i := 0; i < t4Width; i++ {
		mustSetHex(&internalMatrixDiagonalT4[i], internalMatrixDiagonalHex[i])
	}

	// Round constants: full rounds 0..3
	for r := 0; r < 4; r++ {
		for i := 0; i < t4Width; i++ {
			mustSetHex(&roundConstantsT4[r][i], fullRounds0To3[r][i])
		}
	}
	// Partial rounds 4..59 (lane 0)
	for i, rc := range partialRounds4To59 {
		mustSetHex(&roundConstantsT4[4+i][0], rc)
	}
	// Full rounds 60..63
	for r := 0; r < 4; r++ {
		for i := 0; i < t4Width; i++ {
			mustSetHex(&roundConstantsT4[60+r][i], fullRounds60To63[r][i])
		}
	}
}

func mustSetHex(e *fr.Element, s string) {
...
// externalLinearLayerT4 applies the external matrix M_E for t=4.
func externalLinearLayerT4(state *[t4Width]fr.Element) {
	// t0 = s0 + s1, t1 = s2 + s3
	var t0, t1 fr.Element
	t0.Add(&state[0], &state[1])
	t1.Add(&state[2], &state[3])

	// t2 = 2*s1 + t1, t3 = 2*s3 + t0
	var t2, t3 fr.Element
	t2.Double(&state[1])
	t2.Add(&t2, &t1)
	t3.Double(&state[3])
	t3.Add(&t3, &t0)
...
// internalLinearLayerT4 applies the internal matrix M_I for t=4:
// M_I = diag(d0..d3) + ones matrix.
func internalLinearLayerT4(state *[t4Width]fr.Element) {
	var sum fr.Element
	...
	for i := 0; i < t4Width; i++ {
		state[i].Mul(&state[i], &internalMatrixDiagonalT4[i])
		state[i].Add(&state[i], &sum)
	}
}

// permutationT4 performs the Poseidon2 permutation for t=4.
func permutationT4(state *[t4Width]fr.Element) {
	// Initial external linear layer
	externalLinearLayerT4(state)

	// First RF/2 = 4 full rounds
	for r := 0; r < 4; r++ {
		for i := 0; i < t4Width; i++ {
			state[i].Add(&state[i], &roundConstantsT4[r][i])
		}
		for i := 0; i < t4Width; i++ {
			sboxT4(&state[i])
		}
	...
	// Partial rounds
	for r := 0; r < t4PartialRounds; r++ {
		roundIdx := r + 4
		state[0].Add(&state[0], &roundConstantsT4[roundIdx][0])
		sboxT4(&state[0])
		internalLinearLayerT4(state)
	}
	...
	for r := 0; r < 4; r++ {
		roundIdx := r + 60
		for i := 0; i < t4Width; i++ {
			state[i].Add(&state[i], &roundConstantsT4[roundIdx][i])
		}
		for i := 0; i < t4Width; i++ {
			sboxT4(&state[i])
		}
...
// spongeT4 implements the Noir-compatible sponge construction.
type spongeT4 struct {
	state       [t4Width]fr.Element
	cache       [t4Rate]fr.Element
	cacheSize   int
	squeezeMode bool
}

// newSpongeT4 creates a new sponge with IV = inputLen * 2^64.
func newSpongeT4(inputLen int) *spongeT4 {
	s := &spongeT4{}
	s.state[3].SetUint64(uint64(inputLen))
	s.state[3].Mul(&s.state[3], &twoPow64)
	return s
}

func (s *spongeT4) performDuplex() {
	for i := 0; i < s.cacheSize; i++ {
		s.state[i].Add(&s.state[i], &s.cache[i])
	}
	permutationT4(&s.state)
	s.cacheSize = 0
	for i := 0; i < t4Rate; i++ {
		s.cache[i].SetZero()
	}
...
func (s *spongeT4) absorb(input fr.Element) {
	if s.squeezeMode {
		panic("cannot absorb after squeeze")
	}
	if s.cacheSize == t4Rate {
		s.performDuplex()
	}
	s.cache[s.cacheSize] = input
	s.cacheSize++
}

func (s *spongeT4) squeeze() fr.Element {
	if s.squeezeMode {
		panic("already squeezed")
	}
	s.performDuplex()
	s.squeezeMode = true
	return s.state[0]
...
// HashT4Sponge hashes N inputs using the Noir-compatible Poseidon2T4 sponge and returns s0.
func HashT4Sponge(values ...*big.Int) *big.Int {
	sponge := newSpongeT4(len(values))
	var elem fr.Element
...
// Hash2T4 hashes two field elements using Poseidon2T4 sponge (len=2).
// This is the Merkle node hash for note trees.
func Hash2T4(left, right *big.Int) *big.Int {
	return HashT4Sponge(left, right)
}

// Hash3T4 hashes three field elements using Poseidon2T4 sponge (len=3).
func Hash3T4(a, b, c *big.Int) *big.Int {
	return HashT4Sponge(a, b, c)
}

// HashDomainSeparated prepends a domain separator to the hash inputs.
// Domain separation convention for this codebase:
//
//	H(domain, v0, v1, ...) where H is Poseidon2T4 sponge (Noir/Barretenberg compatible).
func HashDomainSeparated(domain *big.Int, values ...*big.Int) *big.Int {
	all := make([]*big.Int, 0, len(values)+1)
	all = append(all, domain)
	all = append(all, values...)
	return HashT4Sponge(all...)
}
